...
impl App {
    ...
    pub(crate) fn help_key(&mut self, key: KeyEvent) {
        ...
        let last = rows(h.scope).len().saturating_sub(1);
        match key.code {
            KeyCode::Esc | KeyCode::Char('q') | KeyCode::Char('?') => self.overlay = Overlay::None,
            KeyCode::Down | KeyCode::Char('j') => h.scroll = (h.scroll + 1).min(last),
            KeyCode::Up | KeyCode::Char('k') => h.scroll = h.scroll.saturating_sub(1),
            KeyCode::Char('d') if ctrl => h.scroll = (h.scroll + 10).min(last),
            KeyCode::Char('u') if ctrl => h.scroll = h.scroll.saturating_sub(10),
            KeyCode::PageDown | KeyCode::Char(' ') => h.scroll = (h.scroll + 10).min(last),
    ...
    pub(crate) fn help_scroll(&mut self, d: isize) {
        ...
        let last = rows(h.scope).len().saturating_sub(1);
        h.scroll = (h.scroll as isize + d).clamp(0, last as isize) as usize;
    }
}
...
mod tests {
    ...
    /// Scrolling stops at both ends rather than running off the list.
    #[test]
    fn scrolling_is_clamped_to_the_list() {
        ...
        assert_eq!(h.scroll, rows(Mode::Library).len() - 1, "stops at the end");
    }
