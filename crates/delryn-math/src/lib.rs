...
/// Em size in px and the extra scale the raster is produced at — high enough that a
/// display equation downscaled to a few text lines by the image pipeline stays
/// crisp. The displayed size is chosen later by `delryn-media` (math sizing), not
/// here; this only sets the source resolution.
...
/// Render `latex` to a black-on-transparent PNG, disk-cached. Returns `None` when
/// RaTeX can't handle the input — the caller then keeps the Unicode fallback.
pub fn render(latex: &str, style: Style) -> Option<Vec<u8>> {
    let path = cache_path(latex, style);
    if let Ok(bytes) = std::fs::read(&path)
        && !bytes.is_empty()
    {
        return Some(bytes);
    }
    let png = render_uncached(latex, style)?;
...
fn render_uncached(latex: &str, style: Style) -> Option<Vec<u8>> {
...
/// `<config>/math/<16-hex>.png`, keyed by the render version + style + source. The
/// theme is deliberately *not* in the key: the PNG is black ink on transparent and
/// recoloured at display time, so one cache entry serves every theme.
fn cache_path(latex: &str, style: Style) -> PathBuf {
...
mod tests {
    ...
    fn renders_caches_and_degrades() {
        ...
        let png = render("x^2 + \\frac{1}{2}", Style::Display).expect("renders a real equation");
        ...
        assert!(cache_path("x^2 + \\frac{1}{2}", Style::Display).exists());
        let again = render("x^2 + \\frac{1}{2}", Style::Display).expect("served from cache");
        assert_eq!(again, png);
        // Garbage never panics; it returns None or a PNG, but does not crash.
        let _ = render("\\notacommand{{{", Style::Inline);
