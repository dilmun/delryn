...
fn transmit_virtual(img: &DynamicImage, id: u32, is_tmux: bool) -> String {
    // delryn patch: transmit as PNG (f=100) instead of raw RGBA (f=32). PNG is
    // far smaller for figures/diagrams, so the (blocking) write to the terminal
    // doesn't stall on large images. Kitty reads the dimensions from the PNG
    // header, so s/v are omitted.
    ...
    for (i, chunk) in chunks.enumerate() {
        ...
        if i == 0 {
            write!(data, "i={id},a=T,U=1,f=100,t=d,").unwrap();
        }
