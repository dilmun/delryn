...
pub fn emoji_raster_for_char(ch: char, glyph_em_px: f32) -> Option<EmojiRasterStrike> {
    // Check the codepoint *before* touching the font. This is the fallback of last
    // resort in `ratex-render`, reached by any glyph the main faces couldn't draw —
    // so on a maths document it is asked about ordinary symbols, and loading the
    // emoji face to answer "no" cost ~183 MB (Apple Color Emoji), cached in a
    // `OnceLock` for the life of the process, for a book containing no emoji at all.
    if !super::is_emoji_char(ch) {
        return None;
    }
