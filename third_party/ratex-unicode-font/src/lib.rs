...
/// The system font found by scanning paths / `fontdb`, shared by the primary slot
/// (when `RATEX_UNICODE_FONT` is unset) and the fallback slot.
///
/// Both used to reach `discover_system_font` through their own `OnceLock`, which
/// read the *same file* into two separate buffers — on macOS that is 22 MB of exact
/// duplication, held for the life of the process. Sharing one `Arc` costs nothing:
/// the bytes are immutable, and the face index travels with them.
static DISCOVERED_SYSTEM_FONT: OnceLock<Option<(Arc<Vec<u8>>, u32)>> = OnceLock::new();

/// [`discover_system_font`], resolved at most once per process and shared.
fn shared_system_font() -> Option<(Arc<Vec<u8>>, u32)> {
...
pub fn load_fallback_font_arc() -> Option<Arc<Vec<u8>>> {
    SYSTEM_FALLBACK_FONT
        .get_or_init(shared_system_font)
        .as_ref()
        .map(|(bytes, _)| Arc::clone(bytes))
}
...
/// Whether `ch` is one an emoji face would be consulted for.
///
/// Covers the pictographic blocks plus the characters that only *become* emoji in
/// context (the keycap combiner, the emoji variation selector, `™`/`ℹ`). Excludes
/// the general mathematical symbol and arrow ranges, which the KaTeX and system
/// Unicode faces already cover — the point is to answer "not an emoji" without
/// paying to load a ~183 MB colour font to find out.
...
fn load_unicode_fallback_font() -> Option<(Arc<Vec<u8>>, u32)> {
    ...
    // 2. System font discovery (shared with the fallback slot)
    shared_system_font()
}
