...
fn try_blit_emoji_raster_fallback(
    ...
    // Check the codepoint before touching the font. This is the last-resort glyph
    // path, reached by anything the main faces couldn't draw — so on a maths
    // document it is asked about ordinary symbols, and loading the colour face to
    // answer "no" costs ~183 MB (Apple Color Emoji), cached for the life of the
    // process, for a document containing no emoji at all.
    if !ratex_unicode_font::is_emoji_char(ch) {
        return false;
    }
