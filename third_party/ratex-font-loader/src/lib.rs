...
impl FontLoadPlan {
    pub fn for_display_items(items: &[DisplayItem]) -> Self {
        ...
        // Tracked separately from the CJK fallbacks: the emoji font is enormous
        // (Apple Color Emoji is ~183 MB) and is cached for the life of the process,
        // so it must not be pulled in merely because *some* character lacked KaTeX
        // metrics. A maths document containing one `−` would otherwise load it.
        let mut needs_emoji_fallback = false;
        ...
        for item in items {
            ...
            } = item
            {
                if let Some(font_id) = FontId::parse(font) {
                    ...
                    if font_id == FontId::EmojiFallback || is_emoji_char(*char_code) {
                        needs_emoji_fallback = true;
                    }
...
/// Whether `char_code` is one an emoji font would be consulted for — the same test
/// the raster fallback uses, so the plan and the renderer agree.
fn is_emoji_char(char_code: u32) -> bool {
    char::from_u32(char_code).is_some_and(ratex_unicode_font::is_emoji_char)
}
